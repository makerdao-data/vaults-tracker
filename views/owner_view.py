...
def owner_page_data(sf, owner_id):
    ...
    try:
        ...
        for vault in vaults_records:
            ...
            vault[0] = link(
                vault[0], "/vault/%s" % vault[0], "Vault %s history" % vault[0]
            )
            if token not in tokens:
                tokens[token] = dict(
                    locked_amount=0, locked_value=0, available_collateral=0
                )
            ...
            vault[1] = (
                link(
                    vault[1], "/collateral/%s" % vault[1], "Vaults using %s" % vault[1]
                )
                if vault[1]
                else ""
            )
        ...
        locked_amounts = sorted(
            [(_t, _v["locked_amount"]) for _t, _v in tokens.items()],
            key=lambda _i: _i[1],
            reverse=True,
        )
        available_collaterals = sorted(
            [(_t, _v["available_collateral"]) for _t, _v in tokens.items()],
            key=lambda _i: _i[1],
            reverse=True,
        )
        locked_amounts = [
            (token, "{0:,.2f}".format(amount))
            for token, amount in locked_amounts
            if amount
        ]
        available_collaterals = [
            (token, "{0:,.2f}".format(amount))
            for token, amount in available_collaterals
            if amount
        ]
...
def owner_page_view(sf, owner_id):
    ...
    if not Web3.isAddress(owner_id):
        return render_template(
            "unknown.html", object_name="owner", object_value=owner_id
        )
